# Russian and Azerbaijani editions

Added 2026-09-11. Each edition has a homepage, about page, room types, guest reviews, and airport, Forum Trabzon and Farabi hospital pages.

## Editing and building

- Edit copy in `data/locales/ru.json` and `data/locales/az.json`.
- Shared templates: `tools/build-international-pages.js`.
- Six-language routes and actual translation groups: `tools/lib/site-languages.js`.
- Run `npm run build`, then `npm test` and `git diff --check`.
- The language normalizer runs after the page generators. It produces reciprocal HTML-head hreflang, six static navigation links and content-hashed shared assets.
- RU/AZ directories are excluded from the legacy Turkish-to-English/Arabic generator. Never add translation markers to manually maintained Turkish home/room pages: this can cause wholesale regeneration of established EN/AR copy.
- Keep Russian/Azerbaijani locale packs separate from the legacy translations directory. Missing copy must fail validation rather than silently fall back to Turkish.
...
Russian primary phrase: “отель в Трабзоне”; Azerbaijani: “Trabzonda otel”. Dedicated landing pages cover airport, Forum and Farabi intent. These are editorial targeting choices, not measured search-volume claims.

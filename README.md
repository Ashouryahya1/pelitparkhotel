...
The four homepages share a statistics banner generated by
`tools/build-home-stats.js` from `data/home-stats.json`. The hotel supplied
Booking.com 9/10, Google 4.9/5 and 6,000+ completed bookings for publication.
These manually maintained figures are separate from the review excerpts and
are not described as a live feed. Update the source data and run the build
when a figure changes; preserve the rest of each localized homepage.
